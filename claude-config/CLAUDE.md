...
- Installed: `git`, `bash`, `node`, `npm`, `apk`, `curl`, `jq`, `openssl`, `make`, `rg` (ripgrep), `sqlite3`, `zip`/`unzip`, `xz`, `tar`/`gzip`, `docker` (proxied), BusyBox utils (`nc`, `ping`, `nslookup`, `awk`, `tree`, `less`, `top`, `ps`). No `python`/`pip` by design — use `node` for ad-hoc scripts. Anything else: `command -v <x>` first; `apk add --no-cache <pkg>` works but is ephemeral (vanishes on rebuild) — for permanent additions, edit `tmt-ai-code/Dockerfile`.
